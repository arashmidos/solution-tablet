...
public class CustomerDaoImpl extends AbstractDao<Customer, Long> implements CustomerDao
{
    ...
    protected ContentValues getContentValues(Customer entity)
    {
        ...
        contentValues.put(Customer.COL_STORE_LOCATION_TYPE_BACKEND_ID, entity.getStoreLocationTypeBackendId());//10
    ...
    {
        String selection = " " + Customer.COL_STATUS + " = ? " + " AND (" + Customer.COL_BACKEND_ID + " is null or " + Customer.COL_BACKEND_ID + " = 0)";
    ...
    public CustomerDto getCustomerDtoById(Long customerId)
    {
        ...
        String table = getTableName() + " cu " +
                " LEFT OUTER JOIN " + BaseInfo.TABLE_NAME + " bi on bi." + BaseInfo.COL_ID + " = cu." + Customer.COL_ACTIVITY_BACKEND_ID;
    ...
    public List<NCustomerListModel> searchForNCustomers(NCustomerSO nCustomerSO)
    {
        ...
        if (Empty.isNotEmpty(nCustomerSO.getSent()))
        {
            ...
            {
                selection = selection.concat(" AND " + Customer.COL_BACKEND_ID + " is not null AND " + Customer.COL_BACKEND_ID +
                        " != 0");
            } else
            {
                selection = selection.concat(" AND " + Customer.COL_BACKEND_ID + " is  null OR " + Customer.COL_BACKEND_ID +
                        " = 0");
            }
        }
        ...
        Cursor cursor = db.query(Customer.TABLE_NAME, projection, selection, argList.toArray(args), null, null, orderBy);
