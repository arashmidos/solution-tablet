...
public class SaleOrderDaoImpl extends AbstractDao<SaleOrder, Long> implements SaleOrderDao
{
    ...
    public List<SaleOrder> retrieveSaleOrderByStatus(Long statusId)
    {
        ...
        String sql = "SELECT * FROM " + SaleOrder.TABLE_NAME + " WHERE " + SaleOrder.COL_STATUS + " = ?";
    ...
    public List<SaleOrderListModel> searchForOrders(SaleOrderSO saleOrderSO)
    {
        ...
        String sql = "SELECT " +
                ...
                " INNER JOIN " + Customer.TABLE_NAME + " c on c." + Customer.COL_BACKEND_ID + " = o." + SaleOrder.COL_CUSTOMER_BACKEND_ID +
    ...
    @Override
    public SaleOrderDto getOrderDtoByCustomerBackendIdAndStatus(Long customerBackendId, Long statusId)
    {
        CommerDatabaseHelper databaseHelper = CommerDatabaseHelper.getInstance(getContext());
        SQLiteDatabase db = databaseHelper.getReadableDatabase();
        String selection = SaleOrder.COL_CUSTOMER_BACKEND_ID + " = ? AND " + SaleOrder.COL_STATUS + " = ?";
    ...
    public void deleteByCustomerBackendIdAndStatus(Long customerBackendId, Long statusId)
    {
        CommerDatabaseHelper databaseHelper = CommerDatabaseHelper.getInstance(getContext());
        SQLiteDatabase db = databaseHelper.getWritableDatabase();
        String sql = " DELETE FROM " + SaleOrder.TABLE_NAME + " WHERE " + SaleOrder.COL_CUSTOMER_BACKEND_ID + " = ? AND " +
                " " + SaleOrder.COL_STATUS + " = ?";
        String[] args = {String.valueOf(customerBackendId), String.valueOf(statusId)};
